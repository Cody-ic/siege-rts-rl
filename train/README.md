...
每一阶段从当前前缀对应的合法值中采样。位置头复用区域特征和坐标，不逐个命令运行网络。
...
当前网络默认约 15.5 万参数。上述 173,671 候选局势下，CPU 单线程预热后一次选择约
11.8 ms（不含候选生成）；这是单次部件测量，不是训练吞吐基准。候选原表占 2.78 MB，
保存的条件集合大小随所选分支变化：采集建筑分支很小，普通建造仍需保存全部可用位置。
测试覆盖小动作集联合概率归一化、候选乱序不改变概率、采样与重算一致、梯度更新、
权重/优化器/随机状态恢复后的下一步精确一致，以及真实游戏连续六次合法命令执行。

当前尚需补齐宏观训练循环、完整对局采样恢复和独立评估。网络单步可训练与权重恢复测试
不代表守方已学会守城，也不代表完整训练可断点续跑。按新版守方设计，宏观模型不再承担
