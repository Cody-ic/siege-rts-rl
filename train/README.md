...
不死鸟战斗阵亡、被击杀守军的当前等级补充金币报价，以及守方实际预付维修木材。
补充报价不等于已实际补兵；维修支出也可能来自地图初始残血，不能全部归因于攻方。
历史报告没有的新字段记为未知。旧 `scouts_killed` 保留“全部非战斗单位”的历史语义，
精确侦察统计请使用 `scout_units_killed`。新诊断字段奖励系数均为零，避免未经标定改目标。

新 tally 布局进入检查点契约；旧 run 必须保留原源码/绑定恢复，不能跳过校验。
现有冻结策略观测和动作形状未改变，但跨版本比较仍应记录原生版本。

实际游戏攻方脚本与模型的连续波次对照使用：

```sh
python train/attacker_campaign_evaluate.py --model runs/export/policy.onnx --maps game/data/maps/pool/gen_01007000.json game/data/maps/pool/gen_01008000.json game/data/maps/pool/gen_01009000.json --output runs/campaign-comparison
```

需要启用 ONNX 的绑定和通过导出验证的模型。每对从同一城市状态开始，守方由游戏脚本
控制，城市/资源跨波保留；报告逐波记录双方 tally 和期末城市。默认观察六波、两个种子，
