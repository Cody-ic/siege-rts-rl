...
public:
    ...
    // 注册一张**真实图片**的地表标记（decal）：用于资源点标记
    // （`StonePt`/`WoodPt`/`GoldPt`）——它们不是实体（不进花名册、不进回放），
    // 精灵流水线（`tools/sprite_gen/`，走 Blender）不为它们出图；而渲染层
    // 必须能让玩家**看见**资源点在哪，否则「金矿明明在地图上」与「玩家眼里
    // 完全没有金矿」同时成立（2026-09-01 试玩反馈）。
    //
    // **2026-09-01 再续：三种资源点从程序化生成的抽象菱形换成真实素材**
    // （树/石堆/矿场）——用户直接给了三张裁好的 PNG，按名字对应资源种类。
    // 存放位置**与流水线精灵同一目录**（`tools/sprite_gen/out_3d/`，
    // `decal_` 前缀区分），不另立一层「与 sprite_dir 同级」的目录假设——
    // 那条假设在 `render_utf8_path` 测试（`--sprites` 指向复制出来的自定义
    // 目录）下不成立，调用方传入的 path 因此应当从 `sprite_dir` 直接拼接。
    // 图片须已经**裁到内容边界**（无多余透明留白），地面锚点取**画布底边
    // 中点**：这批素材的可见内容贴着画布底边，不像 Blender 渲染那批留了一圈
    // margin（`stand_lift_px` 那套目视系数管的是后一种，这里不适用）。
    //
    ...
    void register_decal_image(std::string_view ident, const std::string& path);
    ...
    // 「线性结构真正用到的那些朝向，内容确实落在锚点上」——把
    // `game::SceneModel::run_direction` 那张走向表的**像素前提**变成一条会红的检查。
    // 返回查过的 (标识符, 朝向) 对数。**要 GL 上下文**（走 `get()` 载图）。
    //
    // 背景：四个朝向**共用同一个 `ground_anchor`**（元数据逐 ident 一个、不分朝向），
    // 而这套墙板各朝向的内容偏离锚点的量不同。实测（px）：`SE`/`NE` 是正的
    // （`Wall` 0.0、`Gate` +5.5），`SW`/`NW` 偏了（`Wall` +34.5 且右侧被画布切掉
    // 228 px、`Gate` −32.5）。于是一条水平墙线上门与墙**朝相反方向各偏一段**，
    // 玩家看到的是「城门没和城墙对齐」，而墙本身还被切掉一截。
    //
    // **这种错画面照样出、只是画错了**，没有任何别的测试会红——同
    // `verify_stand_geometry` 挡的那两种硬伤。所以这里把它钉住：
    //
    //   * 内容中心的 x 必须落在锚点附近（容差 `kAnchorTolPx`）
    //   * 内容不得压在画布左右边缘（压着 = 被切掉了一截）
    //
    // 查的是 `game/` **实际会用的**那两个朝向（`SE` 与 `NE`），不是全部四个：
    // `SW`/`NW` 本来就是脏的，全查会让这条恒红。哪两个在用是 `run_direction`
    // 的知识，此处写成常量并在 `tests/scene_model_test.cpp` 那侧对应钉住枚举值
    // ——两边合起来才是完整判据（§7 不许 `game/` 读像素几何）。
    std::size_t verify_linear_anchor();

    // 内容中心与锚点的容差。`Gate` 实测 +5.5 px（≈0.02 格）是这套素材的现状，
    // 取 12 px 留出余量但仍远小于脏朝向那 32–34 px——**它要能把脏的那组挡下来**，
    // 否则这条检查就白写了。
    static constexpr float kAnchorTolPx = 12.0f;
