...
std::vector<std::string> wrap(const FontSet& font, std::string_view text, float width) {
    ...
    for(std::size_t i=0;i<text.size();) {
        ...
        int bytes=0;
        GetCodepointNext(text.data()+i,&bytes);
...
void ChronicleView::preview(int chapter) noexcept {open=true;chapter_=std::clamp(chapter,0,8);scroll_=0;ending_=0;}
std::vector<std::string_view> ChronicleView::strings() {
    std::vector<std::string_view> out{"圣城档案指挥官日记第任守城指挥官篇按波次解锁尚未解锁返回滚轮翻阅记录节选改编本次远征已解锁继续守护放下武器返回日记文字分支不会改变当前对局"};
...
void ChronicleView::update(Vector2 vp,int reached_wave) {
    ...
    if(chapter_==8 && reached_wave>=80) {
        const float y=l.panel.y+l.panel.height-64;
        if(CheckCollisionPointRec(mouse,{l.body.x,y,150,36})) {ending_=1;scroll_=0;}
        if(CheckCollisionPointRec(mouse,{l.body.x+165,y,150,36})) {ending_=2;scroll_=0;}
...
void ChronicleView::draw(const FontSet& font,Vector2 vp,int reached_wave) {
    ...
    const std::string title=unlocked?std::string(entry.title):"尚未解锁";
    ...
    if(chapter_==8 && unlocked) {
        const std::array<std::string,3> labels{"继续守护","放下武器","返回日记"};
        for(int i=0;i<3;++i) {
    ...
    if(chapter_==8 && unlocked) font.draw("文字分支不会改变当前对局",{l.body.x,bottom-27},18,Color{162,169,164,255});
}
}
