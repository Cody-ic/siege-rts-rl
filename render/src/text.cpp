...
const std::vector<std::string_view>& ui_strings() {
    ...
    static const std::vector<std::string_view> kStrings = {
        "当前版本不支持 RL，请使用正式发布版",
        "RL 模型不兼容，仍使用脚本；详情见日志",
        "点击策略切换   /   两种策略分别保存进度",
        "RL 与脚本协同   /   两种策略分别保存进度",
        "声音已关闭开启设备不可用",
        "圣城守望黄昏圣城无尽围城指挥官日记第任守城指挥官这里太安静了。日记已更新按阅读氛围迎敌",
        "旧版已备份请用继续日记不受影响；射程格仅对空地对局结束日记记录读取失败原文件已保留存档恢复对局选择继续保存请检查目录正在长局需要一些时间请稍候",
        "第波堡垒主攻人口石木金已暂停继续菜单回到堡垒查看全图",
        "已选名右键移动驻墙滚轮缩放提前迎敌征兵等级",
        "指令已提交：已下达移动指令，到达后恢复自主作战已下达驻墙指令",
        "可用：点击建筑查看维修、升级或招募建造中升级中招募中剩余秒",
        "金色边框标记当前建筑生产已完成需工兵到场施工，多人可加速剩余单人工时维修中招募完成，等待空闲出兵位置",
        // 操作提示
        ...
        "波", "已暂停", "守方", "攻方", "石", "木", "金",
        // 免费方向提示。**方位名本身不在这里**——它们是
        // `game::display_name(Compass)`，已由 `all_display_strings()` 机械遍历
        // 覆盖，抄一份到这里反而会漂移（同下面「拼出来的串不进清单」那条）。
        "主攻",
        // 侦查面板（`draw_intel_panel`）。**兵种与建筑的中文名不在这里**——
        // 同上，它们由 `all_display_strings()` 覆盖。这里只放面板自己的字面量。
        // 「×」与「/」是 ASCII/符号，「×」是全角乘号 U+00D7，必须登记。
        "已侦查", "侦查失败", "克：", "×", "/",
        // 波次循环的阶段与败局（draw_battle_hud 的 phase 那一段）。
        // 「敌袭迫近 / 交战」是 2026-09-03 起的二分——开打 ≠ 接战（中间隔着
        // 行军与侦查窗口），判据 `game::combat_engaged`；旧文案「进攻中」退役。
        // 「秒」与「用时」是 2026-09-04 把 tick 换成秒时加的（试玩反馈：
        // tick 是仿真的内部单位，对玩家是噪音）。**这一条正是本文件存在的
        // 理由的一次现场演示**：改完 HUD 忘了登记，`render_battle_shot`
        // 当场红在「要画一个没登记的字符 U+79D2(秒)」。
        "建造", "敌袭迫近", "交战", "堡垒陷落·败", "秒", "用时",
        // 警报横幅（`draw_alert_banner`）：窥使入境常驻警报、斥候回报/阵亡
        // 两种一次性短闪、提前召唤确认闪，各一行标题一行副标题。
        "幽影窥使入境", "击落它，别让它看清你的布防",
        "斥候回报：探到敌军集结", "编成与克制见右上侦查面板",
        "斥候阵亡：侦查失败", "本波编成不明，当心各方向",
        "已提前召唤下一波", "敌军即刻开拔",
        // 交互层（对局里的状态行与弹出菜单）。#57 重开后 B/T/R/TAB 那套模式
        // 键没有了，改成框选 + 点目标弹菜单——下面这些是**拼出来的**串的
        // 组成部分，而拼接结果不在这份清单上（同下面那个「×」）。
        "选中", "右键下令", "右键退出", "召唤下一波", "Esc 菜单",
        "征兵", "维修", "当前",
        // 升级那一行的三种文案（`main.cpp` 的 `push_upgrade` 拼的）。
        //
        // **这三条必须登记在这里，而且这份登记是它们唯一的防线。** 缺一个
        // 码点不是渲错、是画的时候抛（本文件底部 `ensure_loaded`），而三种
        // 文案里有两种只在特定局面出现——「顶到等级上限」与「有工程在进行」
        // ——截图回归测试点不开那个弹窗，所以漏登记不会让任何测试变红，
        // 只会在试玩时点一下就崩。
        //
        // 别指望 `game::all_menu_strings()` 顺带覆盖它们：操作说明里确实也
        // 写了「先升堡垒」，但那是两份各自演进的文案，靠它兜住等于把这里的
        // 正确性押在别处一句话不被改上。
        "升级", "上限，先升堡垒", "有工程在进行",
        "整面升级（左上至右下）（右上至左下）段（石木）资源不足无可升级墙段已安排整面升级；仍需工匠施工",
        // 取消施工与成品拆除两行（`main.cpp` 的 `push_remove`）。
        "取消施工", "全额返还", "拆除", "返还",
        // HUD 状态行的「征兵等级」（编队移除后，「升级编队」那行已随编队
        // 一起消失，这里只剩它）。
        "等级",
        // HUD 资源行的「人口 x/y」与征兵弹窗的「(人口满)」后缀。一条登记
        // 同时盖住两处码点（人/口/满）——登记的是码点不是整串。
        "人口满",
