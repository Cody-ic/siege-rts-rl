...
const std::vector<std::string_view>& ui_strings() {
    ...
    static const std::vector<std::string_view> kStrings = {
        ...
        // 正确性押在别处一句话不被改上。
        "升级", "上限，先升堡垒", "有工程在进行",
        // 「升级编队」那一行（兵种等级上限落地，`main.cpp` 的
        // `push_upgrade_force` 拼的）。同上一组的理由：两种文案里「无合格
        // 单位」只在没有够格单位时出现，截图测试点不开那个组合，漏登记
        // 不会让任何测试变红，只会在试玩时点一下就崩。括号/逗号一律走
        // ASCII，不必额外登记（本文件 FontSet 构造函数：ASCII 可打印区间
        // 无条件全收）。HUD 状态行的「征兵等级」也在这里一起登记。
        "无合格单位", "名合格", "共", "等级",
