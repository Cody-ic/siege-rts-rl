...
def build(n=8):
    canvas = Image.new("RGBA", (TW * n + TW, TH * n + TW), (0, 0, 0, 0))
    ox, oy = canvas.width // 2, TW // 2
    ...
    road = {(3, j) for j in range(n)} | {(i, 4) for i in range(4, n)}
    ash = {(6, 6), (7, 6), (6, 7), (7, 7), (7, 5)}
    ...
    for s in range(2 * n - 1):
        for gi in range(n):
            ...
            if not 0 <= gj < n:
                continue
            k = (gi, gj)
            place("Road" if k in road else "Ash" if k in ash
                  else "Grass2" if (gi + gj) % 3 == 0 else "Grass", gi, gj)

    objs = [
        (0, 0, "TreeBig", {}), (7, 0, "TreeSmall", {}), (1, 7, "TreeBig", {}),
        (5, 7, "RockSmall", {}), (1, 2, "Stump", {}), (6, 1, "RockSmall", {}),
        (3, 1, "Archer", {}), (2, 4, "Spear", {}),
        (4, 2, "Ghoul", {"state": "move", "frame": 8}),
        (5, 3, "Knight", {"state": "move", "frame": 6}),
        (6, 3, "Shade", {}), (3, 6, "Ranger", {"state": "move", "frame": 6}),
    ]
