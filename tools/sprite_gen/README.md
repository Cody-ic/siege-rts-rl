...
```json
{
  "px_per_tile": 128,
  "sprites": {
    "Spear":  { "cn": "铁壁枪卫", "canvas": [166, 210], "ground_anchor": [83.0, 154.9] },
    "Knight": { "cn": "鬼域骑士团", "canvas": [296, 332], "ground_anchor": [148.0, 214.5] },
    "Keep":   { "cn": "领主堡垒", "canvas": [150, 342], "ground_anchor": [75.0, 290.2] }
  }
}
```
...
`out_3d/` 的 76 张精灵（19 个实体 × 4 朝向）与 `_sprite_meta.json` **入库**，理由三条：

1. 交付清单要求「软件源代码**包括输入文件和输出文件**」——精灵既是渲染的输出、也是游戏的输入
2. 体积可忽略：共约 2.9 MB，远小于已入库的 `.glb` 源素材
