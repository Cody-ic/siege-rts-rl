...
def check_v26_wild_blocked_fraction(c):
    ...
    [10, 30]。带内 (x+y) % 6 铺 Forest ≈ 16.7%，落在 strict 区间内所以过；
    一株不铺 0%（低于下界）、% 2 铺 50%（高于上界）都必报；无墙自动通过
    （同第 8/9 条先例）。**三个铺法刻意离两条边界都远**（0 / 16.7 / 50 对
    [0.06, 0.20]），于是调阈值占位值时这一组不会跟着变红——阈值的具体数字
    只写在 `thresholds.json`，这里不复制一份。
    """
    th = thresholds.load().profile("strict")
