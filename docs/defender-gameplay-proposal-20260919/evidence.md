...
| 进攻阶段等队 | 超出步速线时返回Stop，该分支未评估塔火风险 | 可能产生塔下无效等待；需与集结、前摇分开核查 |
...
| 窥使 | 活着且亲眼看到塔/防空/堡垒时立即记快照，再撤离；下一波使用 | 没有守方40%抽签；得手后击落不撤销已获取情报 |
| 攻方统计范围 | 防御建筑主要扫初始城圈，读取攻方迷雾记忆 | 外围塔阵可能漏计；快照不等于该窥使本次新发现 |
