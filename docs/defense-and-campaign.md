...
| 工匠 | 默认选择安全施工位置并避险，普通右键也不强制穿越危险区；新增 Shift 右键指定在建/在修/升级建筑，工匠忽略风险直到该工程结束，详见 [建造交互](construction-interactions.md) |
...
当前采用 **World/16、Stats/13、存档 v4**。不死鸟花名册、冷却、身份与撤离参数写入快照。v2/v3 对局不能直接套用新规则；启动时保留独立 legacy 备份，需用对应旧版继续。永久日记格式不变。
