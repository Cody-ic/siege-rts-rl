...
本次没有运行新的改规则反事实，也没有证明某个数值改动必然改善体验。已有第36、40波撤退对照只能支持此前那个具体分支修复，不能替代第41—69波的完整平衡实验。
...
- [证据与复现说明](evidence.md)：冻结输入、规则版本、哈希核验、统计边界和原始证据归档。

本次分析仅使用冻结副本，未修改游戏程序和规则。新策略的对照实验尚未运行。
