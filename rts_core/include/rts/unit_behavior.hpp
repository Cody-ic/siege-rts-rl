...
// 回放照样逐 tick 一致。归 1c（tracker 是 **#57**；#28 是那条已合并的认领 PR）。
...
public:
    ...
    // 2. **地面状态的单位不能攻击空中目标。** 这一层是无状态的兵种通则，
    //    所以仍返回 false；`World::unit_can_engage` 叠加唯一的世界状态例外：
    //    城墙上已完成驻守的 `Archer` 可以对空，一下墙便失去许可。
    //
