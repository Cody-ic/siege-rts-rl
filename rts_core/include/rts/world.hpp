...
public:
    ...
    // 是这座建筑**当前**的等级。
    //
    // Keep pays its fixed per-level table price. Fortifications use the staged
    // price in StatsTable::fortification_upgrade_cost(); other buildings retain
    // the square-root growth difference with a rising materials floor.
    // Player quotes, batch budgeting, actual debit and destruction accounting
    // all share these methods.
