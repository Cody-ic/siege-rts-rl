...
// 共用的夹具：**池图第一张**（D=40，小图上这两个判据都退化，见各测试注释）
...
TEST_CASE("集结期：Wraith 在建造阶段离开集结点去侦查", "[demo]") {
    ...
    // 建造期（260 tick）里它要明显离开集结点（0.20 格/tick，满程约 52 格；
...
// （`Knight` 0.15 vs `Ram` 0.045，40 格路程能甩开 20 格以上）——那是
// 「一波分三批到、没有压迫感」的成因。
//
// **夹具必须用池图**：demo_skirmish 的集结点离堡垒只有 14 格，最快的单位
// 几十 tick 就接战，散布还没来得及拉开——判据在它身上恒真（假绿）。池图
// D=40 格，无编队时散布必然超限（Knight 0.15 vs Ghoul 0.09，200 tick 就
...
TEST_CASE("编队推进：行军途中前锋不甩开最慢兵种", "[demo]") {
    ...
    for (int t = 0; t < 4000; t += 10) {
        ...
        for (std::size_t s = 0; s < spawns.size() && s < 8; ++s) {
            ...
            // 行军后段：无编队时 Knight 0.15 vs Ghoul 0.09 在 D=40 上甩开
