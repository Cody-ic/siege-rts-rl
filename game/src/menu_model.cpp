...
const std::vector<HelpEntry>& help_entries() {
    ...
    static const std::vector<HelpEntry> kEntries = {
        ...
        {"右键（菜单开着时）", "关闭菜单，不生效"},
        {"N", "提前召唤下一波，不等建造倒计时"},
        {"斥候侦查", "斥候不占人口；到有敌人的集结点掷一次死活，成功后累计本波发现，阵亡不抹去已有报告"},
        {"幽影窥使", "敌侦查机只在你的视野里现形；瞭望塔看得最远，想提前发现就造它"},
        {"空格 / 底部按钮", "暂停 / 继续仿真；底部按钮也可回到堡垒或查看全图"},
        {"Ctrl / Alt + 框选，U", "Ctrl框选普通建筑，Alt框选墙门栅栏；显示总工料后按U批量升一级，仍需工匠施工"},
        {"点墙 / 整面升级", "菜单选择一面直墙，悬停预览范围；同线城门一起升级，缺口停止，拐角选方向；每段升一级"},
...
#endif
        {"窥使情报", "窥使持续观察城防3秒后传出情报；完成前击落可阻断，成功情报影响下一波"},
