...
const std::vector<HelpEntry>& help_entries() {
    ...
    //
    // 兵种等级上限（守方升级轴第三个输出）落地后加了两条：`[`/`]` 调征兵
    // 等级——同 `train_force_sel` 一样是个持久值，只在征兵弹窗里用得上；
    // 征兵弹窗多一行"升级编队"，升级的是"这座训练建筑当前对准的那支编队"
    // 里够格且在场的活着单位，跟新兵进哪支编队用的是同一个旋钮。
    static const std::vector<HelpEntry> kEntries = {
        {"左键拖拽", "框选：矩形内的己方单位进入选中集"},
        {"左键点空地（可建造）", "弹出建造菜单，点一种建筑即放置"},
        {"左键点兵营 / 堡垒",
         "弹出征兵菜单；掉血多一行维修，另有一行升级建筑，还有一行升级编队"},
