...
struct WaveCurve {
    ...
    int phoenix_cap = 5;
    // ——撤离与重生（2026-09-10，#170）——
    //
    // `CLAUDE.md`「空中单位」四条结构约束里，「跨波留存」与「击落后 N 波重生」
    // 此前从未落地：不死鸟出了必死（波次终止判据 + 超时收场都会 `kill_unit`）。
    // 后果是实测的——58/60 波出场、只被击落 12 次，而守方全程 2 座 `Flak`
    // ⇒「AA 的机会成本」那个两难从未成立；塔损耗是阶跃而非渐进
    // （`攻守配平的数学模型.md` §5.1），因为它不会走。
    //
    // 血量低于这个千分比就掉头飞回最近集结点，到达即撤离成功、下一波回来。
    // 取值是**占位**（进待标定清单）：太高则它一触即走、点不掉任何东西，
    // 太低则与「出了必死」无异。
    int phoenix_withdraw_hp_permille = 400;
    // 被击落之后要等几波才重生（满血、当波等级）。这是「击落一次意味着攻方
    // 数波内失去空中打击能力」那条设计的**唯一**落点，也是留存的全部代价。
    int phoenix_respawn_waves = 3;
