...
public:
    ...
    // 墙板是 180° 对称的，所以 `SE` 与 `NW` 拼起来都无缝——上一版只测了无缝，
    // 两个都过，选了 `NW`。但这套素材的 `NE` / `NW` 两张图**内容偏离锚点**
    // （实测 `Wall` 偏 +34 px 且右侧被画布切掉、`Gate` 偏 −33 px），
    // 于是「墙整条浮在格子外」而且**门与墙错开约 0.9 格**——玩家看到的就是
    // 「城门缩在城里、没和城墙连上」。`SE` / `SW` 两张是正的。
    //
    // 所以这不是给素材缺陷打补丁：两个朝向本来等价，重测只是在等价的两个里
